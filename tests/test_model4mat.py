...
from resultkit.MatModel import Model4Mat, MatLib, DataType, MatStore
...
def test_color_format_channels():
    C = Model4Mat.ImageMat.ColorFormat
...
def test_shape_type_to_bchw():
    S = Model4Mat.ImageMat.ShapeType
...
def test_numpy_gray_hw_image_is_valid():
    ...
    assert img.shape_type == Model4Mat.ImageMat.ShapeType.HW
...
def test_numpy_rgb_hwc_image_is_valid():
    img = Model4Mat.ImageMat(
        color_format=Model4Mat.ImageMat.ColorFormat.RGB,
        data=np.zeros((10, 20, 3), dtype=np.uint8),
    )

    assert img.lib == MatLib.NUMPY
    assert img.shape_type == Model4Mat.ImageMat.ShapeType.HWC
...
def test_numpy_gray_bhw_batch_is_valid():
    ...
    assert img.shape_type == Model4Mat.ImageMat.ShapeType.BHW
...
def test_numpy_unknown_3d_layout_rejected():
    with pytest.raises(ValueError, match="Cannot infer 3-D NumPy image layout"):
        Model4Mat.ImageMat(
            color_format=Model4Mat.ImageMat.ColorFormat.UNKNOWN,
            data=np.zeros((10, 20, 3), dtype=np.uint8),
        )
...
def test_numpy_bchw_layout_rejected():
    with pytest.raises(TypeError, match="Expected HW, BHW, HWC, or BHWC"):
        Model4Mat.ImageMat(
            color_format=Model4Mat.ImageMat.ColorFormat.GRAY,
            shape_type=Model4Mat.ImageMat.ShapeType.BCHW,
...
def test_torch_bchw_float_image_is_valid():
    ...
    assert img.shape_type == Model4Mat.ImageMat.ShapeType.BCHW
...
def test_numpy_gray_to_torch():
    ...
    assert out.shape_type == Model4Mat.ImageMat.ShapeType.BCHW
...
def test_torch_gray_to_numpy():
    ...
    assert out.shape_type == Model4Mat.ImageMat.ShapeType.BHW
    assert out.BCHW == (1, 1, 2, 3)
...
def test_torch_rgb_to_numpy():
    ...
    assert out.lib == MatLib.NUMPY
    assert out.shape_type == Model4Mat.ImageMat.ShapeType.BHWC
